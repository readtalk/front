...
export default function AuthLayout() {
    ...
    // Check if we're on mobile and if a channel is selected
    const isChannelSelected = location.pathname.startsWith('/channel/');
        ...
        <div className="flex flex-col h-screen">
            ...
            <div className="flex w-full justify-between border-b border-neutral-200 bg-neutral-50 py-1 pr-2 pl-6 transition-colors dark:border-neutral-800 dark:bg-neutral-950">
                <div className="flex flex-1 gap-2 items-center">
                    {/* Back button on mobile when channel is selected */}
                    {isChannelSelected && (
                        <button
                            onClick={() => navigate('/')}
                            className="md:hidden p-1 -ml-2 text-neutral-700 dark:text-neutral-300 hover:bg-neutral-200 dark:hover:bg-neutral-800 rounded"
                        >
                            ←
                        </button>
                    )}
                    <Chats size={16} weight="fill" />
                    <div className="text-sm sm:text-base">READ<strong>Talk</strong></div>
                </div>
                ...
                <div className="flex flex-row-reverse flex-1 items-center gap-3">
                    <div className="relative">
                        <button
                            ...
                            className="size-7 sm:size-8 font-medium cursor-pointer bg-white dark:bg-neutral-900 hover:bg-neutral-200 hover:dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-800 transition-all rounded flex items-center justify-center"
                        >
                            {userInitials}
                        </button>
            ...
            <div className="flex flex-1 overflow-hidden">
                {/* Sidebar - always visible on desktop, full width on mobile when no channel selected */}
                <div className={`
                    ${isChannelSelected ? 'hidden md:block' : 'block'} 
                    w-full md:w-56 px-3 pt-6 
                    ${isChannelSelected ? '' : 'border-r'} 
                    border-neutral-200 transition-colors dark:border-neutral-800
                    overflow-y-auto
                `}>
                    <SidebarContent />
                </div>

                {/* Main Content - hidden on mobile when no channel selected */}
                <div className={`
                    ${!isChannelSelected ? 'hidden md:flex' : 'flex'} 
